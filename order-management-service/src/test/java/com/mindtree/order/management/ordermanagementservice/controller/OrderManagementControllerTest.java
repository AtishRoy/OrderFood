...
public class OrderManagementControllerTest {
	...
	public void testFetchOrder() {
		try {
			...
			Mockito.when(serviceMock.fetchOrder(ArgumentMatchers.anyLong())).thenReturn(order);
	...
	public void testFetchOrderWithNotFoundException() {
		try {
			Mockito.when(serviceMock.fetchOrder(ArgumentMatchers.anyLong()))
	...
	public void testFetchAllOrders() {
		try {
			...
			Mockito.when(serviceMock.fetchAllOrders(ArgumentMatchers.anyInt(), ArgumentMatchers.anyInt())).thenReturn(orders);
	...
	public void testFetchAllOrdersWithNotFoundException() {
		try {
			Mockito.when(serviceMock.fetchAllOrders(ArgumentMatchers.anyInt(), ArgumentMatchers.anyInt()))
	...
	public void testPlaceOrder() {
		try {
			...
			Transaction transaction = new Transaction(1212l, "3FE4DDA55F99EEA2", PaymentMode.CARD, PaymentStatusType.SUCCESS);
			Order order = new Order(1111l, 8689283L, "8888", 9999L, items, 100F, "Address1", "9087654321", OrderStatusType.PLACED, transaction);
			Mockito.when(serviceMock.placeOrder(ArgumentMatchers.any(Order.class))).thenReturn(order);
	...
	public void testCancelOrder() {
		try {
			Mockito.when(serviceMock.cancelOrder(ArgumentMatchers.anyLong())).thenReturn("Order cancelled successfully");
	...
	public void testCancelOrderForAlreadyCancelled() {
		try {
			Mockito.when(serviceMock.cancelOrder(ArgumentMatchers.anyLong())).thenReturn("Order is already cancelled");
			RequestBuilder requestBuilder = MockMvcRequestBuilders.delete("/order/1111")
					.accept(MediaType.APPLICATION_JSON);
			MvcResult result = mockMvc.perform(requestBuilder).andReturn();
			Assert.assertNotNull(result);
	...
	public void testCancelOrderWithNotFoundException() {
		try {
			Mockito.when(serviceMock.cancelOrder(ArgumentMatchers.anyLong())).thenThrow(new OrderNotFoundException("The requested order is not found"));
			RequestBuilder requestBuilder = MockMvcRequestBuilders.delete("/order/1111")
					.accept(MediaType.APPLICATION_JSON);
			MvcResult result = mockMvc.perform(requestBuilder).andReturn();
			Assert.assertNotNull(result);
