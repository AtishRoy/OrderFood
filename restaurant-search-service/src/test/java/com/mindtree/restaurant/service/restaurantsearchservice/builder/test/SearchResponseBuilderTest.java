...
public class SearchResponseBuilderTest {
	...
	private void assertresponseDetails(SearchRestaurantResponseVO response, boolean isAdressAdded) {
		...
		Assert.assertEquals("Delicious pizza",
				response.getRestaurantDetails().get(0).getItemsAvailableList().get(0).getItemDesc());
		Assert.assertEquals("VEG surepeme",
				response.getRestaurantDetails().get(0).getItemsAvailableList().get(0).getItemName());
		Assert.assertEquals("200.0",
				response.getRestaurantDetails().get(0).getItemsAvailableList().get(0).getItemPrice());
		...
		if (isAdressAdded) {
			Assert.assertEquals("#58,1st main road,parvathipuram,Bangalore,560004,Sajjan Rao,Karanataka",
					response.getRestaurantDetails().get(0).getRestaurantAddress());
		}
	}
